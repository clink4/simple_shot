...
import conductance as conductance
...
def train(train_loader, model, criterion, optimizer, epoch, scheduler, log):
    ...
    for i, (input, target) in enumerate(tqdm_train_loader):
        #print("in train; size of input is ", input.size())
        #print("in train; size of target is ", target.size())
        ...
        if args.beta > 0 and r < args.cutmix_prob:
            ...
            output = model(input)
            #print("in train; size of output is ", output.size())
            if args.do_meta_train:
                ...
                #print("in meta_train; size of query proto is ", query_proto.size())
                #print("in meta_train; size of shot proto is ", shot_proto.size())
                for index, item in enumerate(shot_proto):
                    shot_proto[index] = quantize(item, 4, True)
                for index, item in enumerate(query_proto):
                    query_proto[index] = quantize(item, 4, True)
                output = -get_metric(args.meta_train_metric)(shot_proto, query_proto)
                #print("in meta_train; size of output is ", output.size())
...
def meta_evaluate(data, train_mean, shot):
    ...
    for _ in warp_tqdm(range(args.meta_test_iter)):
        ...
        #print("in meta_evaluate; train_data size is ", train_data.shape)
        #print("in meta_evaluate; train_label size is ", len(train_label))
        #print("in meta_evaluate; test_data size is ", test_data.shape)
        #print("in meta_evaluate; test_label size is ", len(test_label))
...
def metric_class_type(gallery, query, train_label, test_label, shot, train_mean=None, norm_type='CL2N'):
    ...
    print("in metric_class_type; distance1 is ", distance)
    print("size of distance1 is ", distance.shape)

    #quantization
    for index, item in enumerate(gallery):
        gallery[index] = quantize(item, args.quantization)

    for index, item in enumerate(query):
        query[index] = quantize(item, args.quantization)

    dist = np.zeros((5, 75))
    for gallery_index, gallery_item in enumerate(gallery):
        for query_index, query_item in enumerate(query):
            dist[gallery_index][query_index] = math.sqrt(np.dot(gallery_item, gallery_item) - 2. * np.dot(gallery_item, query_item) + np.dot(query_item, query_item))
    print("distance2 is ", dist)

    distance3 = sklearn.metrics.pairwise.euclidean_distances(gallery, query)
    print("distance3 is ", distance3)
    print("shape of distance3 is ", distance3.shape)


    '''distance = 1. - sklearn.metrics.pairwise.cosine_similarity(gallery, query) #cosine
    distance = ((gallery[:, None, :] - query)**2).sum(2) #euc squared
    distance = sklearn.metrics.pairwise.manhattan_distances(gallery, query) #man
    distance = sklearn.metrics.pairwise_distances(gallery, query, metric='chebyshev') #cheb
    test = LA.norm(gallery[:, None, :] - query, ord=1, axis=2)'''
    '''distance = np.zeros((5,75))
    for gallery_index, gallery_item in enumerate(gallery):
        for query_index, query_item in enumerate(query):
            #g = np.zeros(len(gallery_item))
            #for i in range(len(gallery_item)):
                #quant_bits = 4 #TODO: change
                #if quant_bits == 4:
                #    g[i] = conductance.G_4bit[int(gallery_item[i])][int(query_item[i])] #TODO:: idk order ://
                #if quant_bits == 3:
                #    g[i] = conductance.G_3bit[int(gallery_item[i])][int(query_item[i])]
            g = np.fromiter((conductance.G_4bit[int(gallery_item[i])][int(query_item[i])] for i in range(len(gallery_item))), float)   #LC faster??
            d = np.sum(g)
            distance[gallery_index][query_index] = d
            #print("dist line is", 1. - sklearn.metrics.pairwise.cosine_similarity(gallery_item, query_item))
